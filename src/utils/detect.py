...
def track(stframe, cap, model, conf=0.4):
    while cap.isOpened():
...
def track_players(stframe, cap, model, team1, team2, conf=0.4):
        ...
        if success:
            ...
            for result in results:
                ...
                for detection, label_in in zip(detections, labels):
                    ...
                    frame = cv2.rectangle(frame, (x1, y1), (x2, y2), dom_color,
                                          2)

                    if label == 'Player':
                        frame = cv2.putText(frame, f"{team_name} {label}",
                                            (x1, y1 - 10),
                                            cv2.FONT_HERSHEY_SIMPLEX, 0.9,
                                            dom_color, 2)
                    else:
                        frame = cv2.putText(frame, f"{label}", (x1, y1 - 10),
                                            cv2.FONT_HERSHEY_SIMPLEX, 0.9,
                                            dom_color, 2)
            # annotated_frame = results[0].plot()
            # annotated_frame = cv2.cvtColor(annotated_frame, cv2.COLOR_BGR2RGB)
